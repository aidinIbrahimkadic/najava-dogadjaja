...
/** —— NEW: Share helpers —— */
function getCanonicalUrl(event) {
  if (typeof window === 'undefined') return '';
  // Ako postoji trenutni URL, koristi njega; fallback na kanoničku rutu po ID-u:
  const current = window.location?.href || '';
  if (current) return current;
  const origin = window.location?.origin || '';
  return `${origin}/dogadjaj/${event?.idguid ?? ''}`;
}

function openPopup(url, name = 'share') {
  const w = 650;
  const h = 540;
...
export default function SingleEvent({ event }) {
  ...
  /** —— NEW: compute share links —— */
  const { shareUrl, shareText, shareLinks, hasNativeShare } = useMemo(() => {
    const url = getCanonicalUrl(event);
    const title = event?.title || 'Događaj';
    const place = event?.lokacija?.naziv ? ` @ ${event.lokacija.naziv}` : '';
      ...
      <PageWrap>
        ...
        <div style={{ height: '100%' }}>
          <InfoCard>
            ...
            {/* —— NEW: Share —— */}
            <ShareSection>
              <ShareTitle>Podijeli</ShareTitle>
              <ShareBar>
                <ShareButton
                  aria-label="Podijeli na Facebook"
                  onClick={() => openPopup(shareLinks.facebook, 'fb-share')}
                >
                  <Fa.FaFacebook /> Facebook
                </ShareButton>

                <ShareButton
                  aria-label="Podijeli na X/Twitter"
                  onClick={() => openPopup(shareLinks.twitter, 'tw-share')}
                >
                  <Fa.FaTwitter /> X / Twitter
                </ShareButton>

                <ShareButton
                  aria-label="Pošalji na WhatsApp"
                  onClick={() => openPopup(shareLinks.whatsapp, 'wa-share')}
                >
                  <Fa.FaWhatsapp /> WhatsApp
                </ShareButton>

                <ShareButton
                  aria-label="Podijeli na Telegram"
                  onClick={() => openPopup(shareLinks.telegram, 'tg-share')}
                >
                  <Fa.FaTelegramPlane /> Telegram
                </ShareButton>

                <ShareButton
                  aria-label="Podijeli na LinkedIn"
                  onClick={() => openPopup(shareLinks.linkedin, 'li-share')}
                >
                ...
                <ShareButton aria-label="Kopiraj link" onClick={() => copyToClipboard(shareUrl)}>
                  <Fa.FaLink /> Kopiraj link
                </ShareButton>
