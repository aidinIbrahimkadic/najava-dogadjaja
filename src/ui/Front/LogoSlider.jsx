...
const hasLogo = (s) =>
  typeof s === 'string' && s.trim().length > 0 && !/^(null|undefined)$/i.test(s.trim());
...
export default function LogoSlider({
  ...
  // 1) Uvijek pozovi HOOKOVE (nema early return-a prije ovoga!)
  const institutionsData = useMemo(() => {
    if (!Array.isArray(allInstitutions)) return [];
    return allInstitutions
      .filter((inst) => hasLogo(inst?.logo))
      .map((institution) => ({
        label: institution.naziv,
        color: institution.boja_pozadine_postera,
  ...
  const sequence = useMemo(() => {
    ...
    const repeats = Math.max(4, Math.ceil(24 / institutionsData.length));
    return Array.from({ length: repeats }, () => institutionsData).flat();
  }, [institutionsData]);
  ...
  useEffect(() => {
    if (!trackRef.current) return;
    halfWidthRef.current = trackRef.current.offsetWidth / 2;
  }, [sequence.length, size, gap]);
  ...
  const onPointerMove = (e) => {
    if (!dragging) return;
    const delta = e.clientX - startX.current;
    if (Math.abs(delta) > 5) setIsDragMove(true);
    let next = startOffset.current + delta;
    next = normalizeOffset(next);
    offsetRef.current = next;
    applyOffset(next);
  ...
  const endDrag = (e) => {
    ...
    try {
      ...
      console.debug('Pointer capture release issue:', err);
    }
  };
  ...
  // 2) Tek sada je OK uslovno “return null”
  if (institutionsData.length === 0) return null;
